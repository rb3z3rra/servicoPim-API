...
describe("Testes CRUD de Equipamentos", () => {
    describe("CREATE - Criar equipamentos", () => {
        test("Deve criar um novo equipamento com sucesso", async () => {
            ...
            mockDataSource.getRepository().create.mockImplementation((data) => ({ ...data }));
            mockDataSource.getRepository().save.mockImplementation(async (entity) => {
                entity.id = idCounter++;
                return entity;
            });
            const result = await equipamentoService.createEquipamento(newEquipment);
        ...
        test("Deve criar múltiplos equipamentos com IDs incrementais", async () => {
            ...
            const equip1 = await equipamentoService.createEquipamento(equip1Data);
            const equip2 = await equipamentoService.createEquipamento(equip2Data);
        ...
        test("Deve lançar erro ao criar equipamento com código já existente", async () => {
            ...
            await expect(equipamentoService.createEquipamento(newEquipment)).rejects.toThrow("Código do equipamento já cadastrado");
        });
    });
    ...
    describe("UPDATE - Atualizar equipamentos", () => {
        test("Deve atualizar equipamento existente com sucesso", async () => {
            ...
            mockDataSource.getRepository().save.mockImplementation(async (entity) => entity);
    ...
    describe("Testes integrados (CRUD completo)", () => {
        test("Deve realizar fluxo completo de CRUD", async () => {
            ...
            const newEquip = await equipamentoService.createEquipamento(newEquipData);
...
//# sourceMappingURL=equipamento.test.js.map